...
static int cache_block(struct scsi_dev_t *device, uint32_t lba) {
    ...
    int ret = device->send_cmd(device->intern_fd, (char *)&read_cmd, sizeof(
                struct scsi_read_10_t), cache->cache, CACHE_BLOCK_SIZE, 0);
    if (ret) return -1;
...
static int scsi_read(int drive, void *buf, uint64_t loc, size_t count) {
    ...
    while (progress < count) {
        ...
        memcpy(buf + progress, &device->cached_blocks[cache].cache[offset], chunk);
        progress += chunk;
    }
...
//	kprint(KPRN_ERR, "SCSI WRITE");
	return -1;
}

int scsi_register(int intern_fd,
        int (*send_cmd) (int, char *, size_t, char *, size_t, int), size_t mtransf) {
    ...
    uint64_t data = 0;
    int ret = send_cmd(intern_fd, (char *)&cap_cmd, sizeof(struct scsi_read_capacity_10_t), (char *)&data, sizeof(uint64_t), 0);
	...
	kprint(KPRN_INFO, "scsi: Initialised /dev/%s with maximum transfer size %X", dev_name, mtransf);
	kfree(dev_name);
	dev.intern_fd = ret;
	dev.size = lba_num * block_size;
	dev.calls.read = scsi_read;
	dev.calls.write = scsi_write;
//	dev.calls.flush = ahci_flush;
	device_add(&dev);
