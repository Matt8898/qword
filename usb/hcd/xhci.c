 ...
 */
static void xhci_enqueue_trb(struct xhci_seg *seg,
                             void *trb, struct xhci_event *ev, int first) {

    //TODO use a raw trb here to make this less confusing.
...
static int xhci_send_command(struct xhci_hcd *controller, uint32_t field1, uint32_t field2, uint32_t field3,
                             uint32_t field4, struct xhci_event *ev) {
...
int xhci_init_device(struct xhci_hcd *controller, uint32_t port, uint32_t speed) {
    xhci_enable_slot(controller);
    if(!controller->slot_id) {
        kprint(KPRN_INFO, "usb/xhci: failed to get slot id.");
    }
    kprint(KPRN_INFO, "usb/xhci: Initializing device, slot id: id: %x port: %x", controller->slot_id, port);
    ...
    kprint(KPRN_INFO, "usb/xhci: sending address device %x %x", xhci_dev->in_ctx.dma_addr, (size_t)slot);
    ksleep(10);
...
int xhci_setup_endpoint(struct usb_dev_t *dev, int epno) {
    ...
    volatile struct xhci_slot_ctx* out_slot = xhci_get_slot_ctx(&xdev->out_ctx, controller->context_size);
    volatile struct xhci_slot_ctx* in_slot = xhci_get_slot_ctx(&xdev->in_ctx, controller->context_size);
    in_slot->field1 = out_slot->field1;
    in_slot->field2 = out_slot->field2;
    in_slot->field3 = out_slot->field3;
    in_slot->field4 = out_slot->field4;

    struct xhci_ep_ctx *ep0_ctx_out = xhci_get_ep0_ctx(&xdev->out_ctx, controller->context_size);
    struct xhci_ep_ctx *ep0_ctx_in = xhci_get_ep0_ctx(&xdev->out_ctx, controller->context_size);
    ...
    kprint(KPRN_INFO, "Endpoint address: %x", dev->endpoints[epno].data.address);
    ...
    kprint(KPRN_INFO, "usb/xhci: setting up endpoint %x, ep number: %x", endpoint_no, x_epno);
    if(out) {//bulk out
        kprint(KPRN_INFO, "usb/xhci: endpoint %x is a bulk endpoint", epno);

        ctrl = xhci_get_control_ctx(&xdev->in_ctx);
        ep_ctx = xhci_get_ep_ctx(&xdev->in_ctx, controller->context_size, (x_epno));
        int val = (2 << 3) | (3 << 1) | (dev->endpoints[epno].data.max_packet_size << 16);
        ep_ctx->field2 = val;
        ...
        ctrl = xhci_get_control_ctx(&xdev->in_ctx);
        ep_ctx = xhci_get_ep_ctx(&xdev->in_ctx, controller->context_size, (x_epno));
        int val = (6 << 3) | (3 << 1) | (dev->endpoints[epno].data.max_packet_size << 16);
        ep_ctx->field2 = val;
...
static void fill_trb_buff(volatile struct xhci_command_trb *cmd,  uint32_t field1,
...
static void fill_setup_trb(struct xhci_command_trb *cmd, struct usb_request_t *req,
                           uint32_t size, int pid) {
    (void)size;
...
static void fill_normal_trb(struct xhci_transfer_trb *trb, void *data,
    ...
    field3 = size;
    field4 = TRB_CMD_TYPE(TRB_NORMAL) | (1 << 2) | (is_last << 5) | (!is_last << 4);
    fill_trb_buff((struct xhci_command_trb *)trb, field1, field2, field3, field4);
}


static int xhci_send_control(struct usb_dev_t *device, char *data,
    ...
    fill_setup_trb(cmd, req, sizeof(*req), 1);
    ...
    if(req->length) {
        pid = 1;
        fill_setup_data(cmd, (void*)((size_t)data - MEM_PHYS_OFFSET), req->length, pid);
        cmd = xhci_get_trb(ctrl);
    }
...
static int xhci_send_bulk(struct usb_dev_t *dev, char *data,
                          size_t size, int epno, int out) {
...
//	irqoff = 1;
    ...
    volatile struct xhci_db_regs *dbr;
    struct xhci_endpoint* xep = ((struct xhci_endpoint*)dev->endpoints[epno].hcd_ep);
    ...
    seg = &xep->seg;
    dbr = controller->db_regs;
    size_t len = 64 * 1024 - (((size_t)data - MEM_PHYS_OFFSET) & (64 * 1024 - 1));
    if(len < size) {
        // kprint(KPRN_INFO, "BOUNDARY CROSSED");
    }
    trb = xhci_get_trb(seg);
    fill_normal_trb(trb, (void*)((size_t)data - MEM_PHYS_OFFSET), size, 1);
    uint32_t index = (seg->enq - (uint64_t)seg->trbs) / 0x10;
    kprint(KPRN_INFO, "usb/xhci: control event index is at %X, enq:%X trbs:%X", index, seg->enq, seg->trbs);
    struct xhci_event ev = {0};;
    seg->seg_events[index] = &ev;
    dbr->db[slot_id] = xep->x_epno;
    event_await(&ev.event);
//    xhci_wait_event(controller, 0);
    trb->addr = 0;
    trb->len = 0;
    trb->flags = 0;
    spinlock_release(&xep->lock);
//	kprint(KPRN_INFO, "BULK OPERATION DONE");
    return 0;
}
        ...
        while((event->flags & 0x1) == controller->ering.cycle_state) {
            ...
            switch(TRB_TYPE(event->flags)) {
                ...
                case 0x20: {
                    ...
                    struct xhci_seg *tseg = controller->xdevs[slot_id]->ep_segs[epid];
                    int index = (event->addr - ((size_t)tseg->trbs - MEM_PHYS_OFFSET)) / 0x10;
                    if(index == 0xfe) {
                        index = -1;
                    }
                    kprint(KPRN_INFO, "usb/xhci: dequeueing event at index %X for %X %X at %X, %X", index, slot_id, epid, tseg->trbs, event->addr);
            ...
            if(*bios_sem) {
                ...
                *os_sem = 1;
                while(1) {
                    ...
                    if(*bios_sem == 0) {
                        kprint(KPRN_INFO, "usb/xhci: device is no longer bios-owned");
                        break;
                    }
                    ksleep(100);
                }
...
void xhci_controller_init(struct pci_device_t* pci_dev) {
    ...
    kprint(KPRN_INFO, "usb/xhci: conroller halted");
...
#define XHCI_ERST_SIZE_MASK 0xFFFF
    uint32_t erstsz = controller->run_regs->irs[0].erstsz & ~XHCI_ERST_SIZE_MASK;
    erstsz = erstsz | controller->erst.num_segs;
    controller->run_regs->irs[0].erstsz = erstsz;

    //Set erstba
#define XHCI_ERST_ADDR_MASK (~(0x3FUL))
    uint64_t erstba = controller->run_regs->irs[0].erstba & ~XHCI_ERST_ADDR_MASK;
...
struct usb_hc_t *usb_init_xhci(void) {
    ...
    int i = 0;
    while(1) {
        struct pci_device_t *pci_dev = pci_get_device(XHCI_CLASS, XHCI_SUBCLASS, PROG_IF, i);
