...
static int mass_storage_send_cmd(int drive, char *cmd, size_t cmd_length,
	...
	if(debug_xhci) {
//		kprint(KPRN_INFO, "MASS STORAGE COMMAND");
	}
//	kprint(KPRN_INFO, "MASS STORAGE COMMAND");
    struct cbw_t cbw = {0};
    ...
    cbw.tag = 1;
//	kprint(KPRN_INFO, "REQ LEN %X", request_length);
	...
	//kprint(KPRN_INFO, "MASS STORAGE 1");
    if (ret) {
        errno = EIO;
        return -1;
    }
	...
	if(debug_xhci) {
//		kprint(KPRN_INFO, "MASS STORAGE COMMAND DONE");
	}
	//kprint(KPRN_INFO, "MASS STORAGE COMMAND COMLPETE");
    return 0;
}
...
int init_mass_storage(struct usb_dev_t *device) {
    ...
    scsi_register(ret, mass_storage_send_cmd, device->endpoints[internal_dev.in].data.max_packet_size);
